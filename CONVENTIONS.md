...
적응형 분기는 운영 서버가 요청 시점에 처리하는 것이 원칙이다. 클라이언트에서 이동하면 검색 엔진과 브라우저가 잘못된 HTML을 먼저 받고 초기 화면도 한 번 더 렌더되므로, `tools/ua-redirect.js`가 생성하는 루트 진입 페이지는 서버 분기를 붙이기 전의 폴백이자 참조 구현으로만 사용한다.

- 최초 선택은 UA 문자열이 아니라 뷰포트 폭을 사용한다. 1024px 이상은 PC, 그 미만은 MO다.
- PC 헤더에는 `모바일 버전으로`, MO 헤더에는 `PC 버전으로` 링크를 둔다. 두 링크는 `../index.html?view=mo|pc`로 루트 폴백을 거치므로 서버와 `file://` 이관본에서 모두 같은 상대 경로를 쓴다.
- 명시 선택은 `publishing-view`라는 `localStorage` 값으로 기억하며 이후 뷰포트 판정보다 우선한다. 저장소가 차단된 환경에서는 현재 뷰포트로 안전하게 되돌아간다.
- 이미 `/pc/` 또는 `/mo/` 경로에 진입한 문서는 자동으로 다시 분기하지 않는다. 양쪽 문서가 서로를 보내는 무한 리다이렉트 루프를 막기 위해서다.
- 운영 서버로 이관할 때는 같은 우선순위(명시 선택 → 저장된 선택 → 뷰포트/서버가 확보한 클라이언트 힌트)를 서버 정책으로 옮기고, 루트 폴백은 보조 경로로만 남기거나 제거한다.
