...
function runTests() {
  const testDir = path.join(SRC, '__tests__');
  if (!walk(testDir, ['.js']).length) return;

  const r = spawnSync(process.execPath, ['--test', testDir], {
    cwd: ROOT,
    encoding: 'utf8',
  });
  if (r.status !== 0) {
    const out = `${r.stdout || ''}${r.stderr || ''}`.trim();
    err('src/__tests__', `node --test 실패\n${indent(out)}`);
  }
}
...
function main() {
  if (!fs.existsSync(SRC)) {
    if (!QUIET) console.log('  src/ 가 아직 없다 — 검사할 대상 없음');
    return;
  }
