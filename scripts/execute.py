...
class StepExecutor:
    ...
    def _load_guardrails(self) -> str:
        sections = []
        # AGENTS.md 가 규칙의 정본이다. CLAUDE.md 는 그것을 가리키기만 하므로
        # 둘 다 읽으면 같은 내용이 두 번 들어간다. 예전 저장소를 위해 폴백만 남긴다.
        agents_md = ROOT / "AGENTS.md"
        legacy_claude_md = ROOT / "CLAUDE.md"
        instructions_file = agents_md if agents_md.exists() else legacy_claude_md
        if instructions_file.exists():
            sections.append(
                f"## 프로젝트 규칙 ({instructions_file.name})\n\n"
                f"{instructions_file.read_text(encoding='utf-8')}"
            )
        docs_dir = ROOT / "docs"
        if docs_dir.is_dir():
    ...
    def _execute_single_step(self, step: dict, guardrails: str) -> bool:
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            ...
            with progress_indicator(tag) as pi:
                invocation = self._invoke_codex(step, preamble)
            ...
            # step 이 status 를 못 남기고 죽었으면(타임아웃 등) 호출 쪽 사유를 쓴다.
            # 그러지 않으면 "Step did not update status" 만 남아 재시도 프롬프트가
            # 원인을 못 알려준다.
            fallback = invocation.get("failure") or "Step did not update status"
            err_msg = next(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Codex Step Executor")
